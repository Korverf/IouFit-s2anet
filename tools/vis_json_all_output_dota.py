...
def draw_poly_detections(img, detections, class_names, putText=True, showStart=False, colormap=None):
    """
...
img_root = '/media/yyw/607ca214-10cb-4af6-af97-eac656e135d1/yyw/yyf/Dataset/DOTA-1.0/data/test/images'
vis_root = '/media/yyw/607ca214-10cb-4af6-af97-eac656e135d1/yyw/yyf/Dataset/DOTA-1.0/vis/retinanet_obb_r50_fpn_1x_dota'
...
pred_list = json.load(open('/media/yyw/607ca214-10cb-4af6-af97-eac656e135d1/yyw/yyf/projects/s2anet/work_dirs/'
                           'retinanet_obb_r50_fpn_1x_dota/retinanet_obb_r50_fpn_1x_dota.json'))
threshold = 0.1  # 0.95
...
for file in pred_list:
    pred_dict = {classes[i]: [] for i in range(15)}
    file_pred_name = file['image_name']
    labels_pred_list = file['labels']
    img_path = os.path.join(img_root, file_pred_name)
    vis_path = os.path.join(vis_root, file_pred_name)

    detections = {classes[i]: np.zeros((0, 9)) for i in range(15)}
    ...
    img = draw_poly_detections(img_path, detections, classes, colormap=colormap, showStart=False, putText=False)
