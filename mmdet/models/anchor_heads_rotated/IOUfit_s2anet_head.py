...
class IOUfitS2ANetHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_odm_bbox=dict(
                     type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)):
        super(IOUfitS2ANetHead, self).__init__()
        ...
        self._init_layers()
        self.fam_IOUfit = IOUfitModule(in_features=16, hidden_features=16)
        self.odm_IOUfit = IOUfitModule(in_features=16, hidden_features=16)
    ...
    def loss_fam_single(self,
        ...
        loss_fam_bbox = torch.zeros(1).cuda(fam_bbox_pred.get_device())
        pos_inds = labels > 0
        if pos_inds.any():
            ...
            pos_fam_bbox_pred = fam_bbox_pred.view(
                    fam_bbox_pred.size(0), 5)[pos_inds]
            pos_poly_target, pos_fam_poly_pred = rotated_box_to_poly(pos_bbox_target), rotated_box_to_poly(pos_fam_bbox_pred)

            #归一化
            x = torch.cat([pos_fam_poly_pred[:, 0::2], pos_poly_target[:, 0::2]], dim=1) #N,8
            xmin, _ = torch.min(x, dim=1)
            xmax, _ = torch.max(x, dim=1)
            y = torch.cat([pos_fam_poly_pred[:, 1::2], pos_poly_target[:, 1::2]], dim=1)  #N,8
            ...
            pos_fam_poly_pred[:, 0::2] = pos_fam_poly_pred[:, 0::2] - xmin.unsqueeze(-1)
            pos_fam_poly_pred[:, 1::2] = pos_fam_poly_pred[:, 1::2] - ymin.unsqueeze(-1)
            pos_fam_poly_pred = pos_fam_poly_pred / long_side.unsqueeze(-1)
            ...
            # caculate IOU as gt
            # IoU_targets = obb_overlaps(pos_fam_bbox_pred, pos_bbox_target.detach(), is_aligned=True).squeeze(1)\
            #    .clamp(min=1e-6, max=1) #未归一化的5参数框
            # use MLP to fit the rotate IOU
            iou_fit_value = self.fam_IOUfit(pos_fam_poly_pred, pos_poly_target) #0~1
    ...
    def loss_odm_single(self,
        ...
        reg_decoded_bbox = cfg.get('reg_decoded_bbox', False)
        assert reg_decoded_bbox==True
        loss_odm_bbox = torch.zeros(1).cuda(odm_bbox_pred.get_device())
        pos_inds = labels > 0
        if pos_inds.any():
            # When the regression loss (e.g. `IouLoss`, `GIouLoss`)
            # is applied directly on the decoded bounding boxes, it
            ...
            pos_bbox_target = bbox_targets[pos_inds.type(torch.bool)]  #
            pos_odm_bbox_pred = odm_bbox_pred.view(
                    odm_bbox_pred.size(0), 5)[pos_inds]
            pos_poly_target, pos_odm_poly_pred = rotated_box_to_poly(pos_bbox_target), rotated_box_to_poly(pos_odm_bbox_pred)

            #归一化
            x = torch.cat([pos_odm_poly_pred[:, 0::2], pos_poly_target[:, 0::2]], dim=1) #N,8
            xmin, _ = torch.min(x, dim=1)
            xmax, _ = torch.max(x, dim=1)
            y = torch.cat([pos_odm_poly_pred[:, 1::2], pos_poly_target[:, 1::2]], dim=1)  #N,8
            ymin, _ = torch.min(y, dim=1) #N
            ymax, _ = torch.max(y, dim=1)
            w = torch.sub(xmax, xmin).unsqueeze(-1)
            h = torch.sub(ymax, ymin).unsqueeze(-1)
            ...
            pos_odm_poly_pred[:, 0::2] = pos_odm_poly_pred[:, 0::2] - xmin.unsqueeze(-1)
            pos_odm_poly_pred[:, 1::2] = pos_odm_poly_pred[:, 1::2] - ymin.unsqueeze(-1)
            pos_odm_poly_pred = pos_odm_poly_pred / long_side.unsqueeze(-1)
            ...
            # caculate IOU as gt
            # IoU_targets = obb_overlaps(pos_odm_bbox_pred, pos_bbox_target.detach(), is_aligned=True).squeeze(1)\
            #    .clamp(min=1e-6, max=1) #未归一化的5参数框
            # use MLP to fit the rotate IOU
            iou_fit_value = self.odm_IOUfit(pos_odm_poly_pred, pos_poly_target) #0~1
            iou_fit_value = iou_fit_value[:, 0].clamp(min=1e-6, max=1)
